...
describe('synchronous-learning-strategy', () => {
  ...
  it('can get the next card', () => {
    ...
    const hpq = mock(new Queue([scoredCard], QUEUE_OPTIONS)).shouldReceive('peek').once().andReturn(scoredCard);
  ...
  it('can move the card from the HPQ to the back of the LPQ', () => {
    ...
    const hpq = mock(new Queue([scoredCard])).shouldReceive('remove').once().andReturn([scoredCard]);
    const lpq = mock(new Queue()).shouldReceive('enqueue').once().with(updatedScoredCard);
  ...
  it('can move the card from the HPQ to the back of the HPQ', () => {
    ...
    const hpq = mock(new Queue([scoredCard])).shouldReceive('remove').once().andReturn([scoredCard]).shouldReceive('enqueue').once().with(updatedScoredCard);
  ...
  it('can move the card from the HPQ to Done', () => {
    ...
    const hpq = mock(new Queue([scoredCard])).shouldReceive('remove').once().andReturn([scoredCard]);
    const done = mock(new Queue()).shouldReceive('enqueue').once().with(updatedScoredCard);
  ...
  it('can refill the HPQ when the HPQ is half empty', () => {
    const learnedThreshold = 1;
    const hpqScoredCard = ScoredCardBuilder.create().withCorrectCount(learnedThreshold).build();

    const lpqScoredCard = ScoredCardBuilder.create().build();
    const backlogScoredCard = ScoredCardBuilder.create().build();

    const hpq = mock(new Queue([hpqScoredCard])).shouldReceive('remove').once().andReturn([hpqScoredCard]).shouldReceive('getLength').once().andReturn(0).shouldReceive('enqueue').twice();

    const lpq = mock(new Queue([lpqScoredCard])).shouldReceive('dequeue').once().andReturn(lpqScoredCard);

    const backlog = mock(new Queue([backlogScoredCard])).shouldReceive('dequeue').once().andReturn(backlogScoredCard);
